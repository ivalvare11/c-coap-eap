...
int main()
{
	...
	BYTE_ARRAY_NEW(CRED_I_out, CRED_I_SIZE, CRED_I_SIZE);
	BYTE_ARRAY_NEW(CRED_R_out, CRED_R_SIZE, CRED_R_SIZE);
...
#define ORIG
	...
	if (TEST_VEC_NUM > 6){
		...
		//NO LONGER NEEDED IN THE NEW VERSION WITH CRED_I, CRED_R
	...
	/*if (TEST_VEC_NUM > 6){
	   ...
	   cred_r.cred.ptr = (uint8_t *)malloc(CRED_R_SIZE);
		...
		if (get_last_creds(&c_i.id_cred_psk, &c_i.cred_i, &cred_r.cred, "../../../test_vectors/psks_i.txt")!=0)
			return -1;
	}*/
	...
	/*
	if (c_i.suites_i.ptr[0] == 0x00) {
		alg = X25519;
	}
	*/

	TRY(ephemeral_dh_key_gen(alg, seed, &X_random, &G_X_random));
	...
	//store_creds(&rKID, &CRED_I_out, &CRED_R_out, "../../../test_vectors/psks_i.txt");
