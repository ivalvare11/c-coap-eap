...
static void * eap_tls_init(struct eap_sm *sm)
{
	...
	#ifdef MEASURE_TIME
    gettimeofday(&endt, NULL);  // Record the end time

    // Calculate the time difference
    double time_taken = (endt.tv_sec - startt.tv_sec) * 1.0 + 
                        (endt.tv_usec - startt.tv_usec) / 1000000.0;

   	total_time_takent += time_taken;
    #endif
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (tls_connection_established(data->ssl_ctx, data->ssl.conn) &&
		...
		// Calculate the time difference
		time_taken = (endt.tv_sec - startt.tv_sec) * 1.0 + 
		            (endt.tv_usec - startt.tv_usec) / 1000000.0;

		total_time_takent += time_taken;

		//printf("TOTAL TIME TAKEN: %f\n", total_time_takent);
		write_file(total_time_takent);
		#endif
