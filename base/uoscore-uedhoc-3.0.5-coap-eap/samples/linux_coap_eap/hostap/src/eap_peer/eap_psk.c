...
#ifdef MEASURE_TIME
struct timeval starttt, endtt;
double total_time_takentt = 0.0;
#endif
...
static void * eap_psk_init(struct eap_sm *sm)
{
	...
	#ifdef MEASURE_TIME
    gettimeofday(&endtt, NULL);  // Record the end time

    // Calculate the time difference
    double time_taken = (endtt.tv_sec - starttt.tv_sec) * 1.0 + 
                        (endtt.tv_usec - starttt.tv_usec) / 1000000.0;

   	total_time_takentt += time_taken;
    #endif
				       ...
				       const struct wpabuf *reqData)
{
	...
	case PSK_INIT:
		...
		// Calculate the time difference
		time_taken = (endtt.tv_sec - starttt.tv_sec) * 1.0 + 
			         (endtt.tv_usec - starttt.tv_usec) / 1000000.0;
