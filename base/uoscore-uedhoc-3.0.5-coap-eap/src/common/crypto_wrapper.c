				...
				struct byte_array *out, struct byte_array *tag)
{
	...
	CRYS_AESCCM_KeySize_t key_id =
		(key->len == 16) ? CRYS_AES_Key128BitSize :
		(key->len == 24) ? CRYS_AES_Key192BitSize :
	...
	PRINTF("== cc310_ccm_auth_crypt: op=%s key=%u nonce=%u aad=%u in=%u out=%u tag=%u ==\n",
	       (op == ENCRYPT) ? "ENC" : "DEC", (unsigned)key->len,
	       (unsigned)nonce->len, (unsigned)(aad ? aad->len : 0),
	       (unsigned)in->len, (unsigned)out->len, (unsigned)tag->len);

	rc = CRYS_AESCCM_Init(&ctx, mode, key_buf, key_id, aad ? aad->len : 0,
			      in->len, // total plaintext/ciphertext length
			      nonce->ptr, nonce->len, tag->len);
	...
	if (full) {
		rc = CRYS_AESCCM_BlockTextData(&ctx, in->ptr, full, out->ptr);
	...
	rc = CRYS_AESCCM_Finish(&ctx, last_in, rem, last_out,
...
static int rs64_to_der(const uint8_t rs[64], uint8_t der_out[80],
		       size_t *der_len)
{
		   ...
		   struct byte_array *out)
{
	if (alg == SHA_256) {
		...
		if (alg == SHA_256) {
			...
			CRYS_HASH_Result_t
				digest_words; // uint32_t[CRYS_HASH_RESULT_SIZE_IN_WORDS]
			...
			memcpy(out->ptr, (const uint8_t *)digest_words,
			   ...
			   struct byte_array *ikm, uint8_t *out)
{
	...
	if (alg == SHA_256) {
		...
		CRYS_HASH_Result_t
			mac_words; // uint32_t[CRYS_HASH_RESULT_SIZE_IN_WORDS]
		CRYSError_t rc;
		...
		memcpy(out, (const uint8_t *)mac_words,
		       CRYS_HASH_SHA256_DIGEST_SIZE_IN_BYTES);
			  ...
			  const struct byte_array *info, struct byte_array *out)
{
	...
	for (uint32_t i = 1; i <= iters; i++) {
		...
		if (info && info->ptr && info->len) {
			rc = CRYS_HMAC_Update(&ctx, info->ptr,
					      (uint32_t)info->len);
			if (rc != CRYS_OK) {
				PRINTF("[CC310][hkdf_expand] Update failed: 0x%08X\n",
				       rc);
...
/* (Sin cambios aquí) */
	...
	if (op == DECRYPT) {
		...
		struct byte_array in_ct = { .ptr = in->ptr,
					    .len = (uint32_t)ct_len };
		struct byte_array in_tag = { .ptr = in->ptr + ct_len,
					     .len = (uint32_t)tag_len };
		struct byte_array pt = { .ptr = out->ptr,
					 .len = (uint32_t)ct_len };

		PRINTF("AEAD:DEC  key=%u nonce=%u aad=%u in=%u(out=%u) tag=%u\n",
		       (unsigned)key_len, (unsigned)nonce_len,
		       (unsigned)aad_len, (unsigned)in_len, (unsigned)ct_len,
		       (unsigned)tag_len);
		...
		if (ct_len == 0) {
			...
			struct byte_array calc_tag = {
				.ptr = calc_tag_buf, .len = (uint32_t)tag_len
			};

			cc310_hw_acquire();
			// CCM MAC-only: ENCRYPT with PT empty generates expected tag
			int rc =
				cc310_ccm_auth_crypt(ENCRYPT, key, nonce, aad,
						     &empty, &empty, &calc_tag);
			cc310_hw_release();
			if (rc != 0) {
			...
			if (ct_memeq_ret0(calc_tag_buf, in_tag.ptr, tag_len) !=
			    0) {
		...
		cc310_hw_acquire();
		int rc = cc310_ccm_auth_crypt(DECRYPT, key, nonce, aad, &in_ct,
					      &pt, &in_tag);
		cc310_hw_release();
		...
		struct byte_array ct = { .ptr = out->ptr,
					 .len = (uint32_t)in_len };

		PRINTF("AEAD:ENC key=%u nonce=%u aad=%u in=%u out_cap=%u tag=%u\n",
		       (unsigned)key->len, (unsigned)nonce->len,
		       (unsigned)(aad ? aad->len : 0), (unsigned)in_len,
		       (unsigned)out->len, (unsigned)tag_len);

		cc310_hw_acquire();
		int rc = cc310_ccm_auth_crypt(ENCRYPT, key, nonce, aad, in, &ct,
					      tag);
		cc310_hw_release();
		   ...
		   uint8_t *out)
{
	...
	if (alg == EdDSA) {
		...
		const CRYS_ECPKI_HASH_OpMode_t hash_mode =
			CRYS_ECPKI_HASH_SHA256_mode;
		// const CRYS_ECPKI_HASH_OpMode_t hash_mode = CRYS_ECPKI_AFTER_HASH_SHA256_mode;

		rc = CRYS_ECPKI_BuildPrivKey(p256, (const uint8_t *)sk->ptr,
					     (uint32_t)sk->len, &user_priv);
		if (rc != CRYS_OK) {
		...
		rc = CRYS_ECDSA_Sign(
			...
			(SaSiRndGenerateVectWorkFunc_t)
				CRYS_RND_GenerateVector, // some SDKs use this alias
			&sign_ctx, &user_priv, hash_mode, (uint8_t *)msg->ptr,
			(uint32_t)msg->len, sig_der, &sig_der_len);
		     ...
		     bool *result)
{
	...
	if (alg == ES256) {
		...
		*result = false;
		...
		else if (sgn->len >= 8 && sgn->ptr[0] == 0x30 &&
			 sgn->len <= sizeof(sig_der)) {
		...
		rc = CRYS_ECPKI_BuildPublKey(p256, pub65, sizeof(pub65),
					     &user_pub);
		if (rc != CRYS_OK) {
			...
			rc = CRYS_ECPKI_BuildPublKey(p256, pub64, sizeof(pub64),
						     &user_pub);
			if (rc != CRYS_OK) {
				   ...
				   uint8_t *shared_secret)
{
	...
	if (alg == P256) {
		...
		// pk 64B (X||Y) transformed into 65B
			...
			rc = CRYS_ECPKI_BuildPublKey(
				pDomain, tmp_pub, sizeof(tmp_pub), &user_pub);
			...
			rc = CRYS_ECPKI_BuildPublKey(pDomain, comp,
						     sizeof(comp), &tmp_pub);

			if (rc != CRYS_OK) {
				comp[0] = 0x03;
				rc = CRYS_ECPKI_BuildPublKey(
					pDomain, comp, sizeof(comp), &tmp_pub);
			}
			...
			rc = CRYS_ECPKI_ExportPublKey(&tmp_pub,
						      CRYS_EC_PointUncompressed,
						      pub65, &out_size);
		...
		rc = CRYS_ECPKI_BuildPrivKey(pDomain, (uint8_t *)sk->ptr,
					     (uint32_t)sk->len, &user_priv);
		if (rc != CRYS_OK) {
		...
		rc = CRYS_ECPKI_BuildPublKey(pDomain, pub65, (uint32_t)pub_len,
					     &user_pub);
		if (rc != CRYS_OK) {
		...
		rc = CRYS_ECDH_SVDP_DH(&user_pub, // PartnerPublKey_ptr
				   ...
				   struct byte_array *sk, struct byte_array *pk)
{
	if (alg == X25519) {
		...
		rc = CRYS_ECPKI_GenKeyPair(
			...
			p256, &user_priv, &user_pub, &kg_temp, &fips_ctx);
		...
		rc = CRYS_ECPKI_ExportPublKey(&user_pub,
					      CRYS_EC_PointUncompressed,
					      pub_uncompressed, &pub_len);
		if (rc != CRYS_OK || pub_len != 65 ||
