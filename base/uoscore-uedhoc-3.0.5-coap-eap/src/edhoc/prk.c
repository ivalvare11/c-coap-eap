...
enum err prk_derive_psk(struct suite suite, uint8_t label,
		    struct byte_array *context, const struct byte_array *prk_in,
		    const struct byte_array *stat_psk, uint8_t *prk_out)
{

	BYTE_ARRAY_NEW(salt, HASH_SIZE, get_hash_len(suite.edhoc_hash));
	TRY(edhoc_kdf(suite.edhoc_hash, prk_in, label, context, &salt));
	PRINT_ARRAY("SALT4e3m", salt.ptr, salt.len);

	BYTE_ARRAY_NEW(cred_psk, stat_psk->len, stat_psk->len);
	byte_array_cpy(&cred_psk, stat_psk, stat_psk->len);

	PRINT_ARRAY("cred_psk for prk derivation", cred_psk.ptr, cred_psk.len);

	TRY(hkdf_extract(suite.edhoc_hash, &salt, &cred_psk, prk_out));
