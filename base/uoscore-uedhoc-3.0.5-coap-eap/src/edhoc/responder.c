...
static inline enum err msg2_encode(const struct byte_array *g_y,
				   ...
				   struct byte_array *msg2, bool is_psk)
{
	uint32_t cph_size = 0;
	if (!is_psk){
		cph_size = G_Y_CIPHERTEXT_2;
	}
	else {
		cph_size = G_Y_CIPHERTEXT_2_PSK;
	}

	BYTE_ARRAY_NEW(g_y_ciphertext_2, cph_size,
		  ...
		  struct byte_array *c_i)
{
	...
	bool static_dh_r;
	authentication_type_get(method, &rc->static_dh_i, &static_dh_r, &rc->is_psk);
	...
	if (!rc->is_psk){
		...
		/*message 2 create*/
		TRY(msg2_encode(&c->g_y, &c->c_r, &ciphertext_2, &rc->msg, rc->is_psk));
	...
	/*for EDHOC-PSK*/
		...
		/*compute ciphertext_2*/
		BYTE_ARRAY_NEW(plaintext_2, PLAINTEXT2_PSK_SIZE,
			       AS_BSTR_SIZE(c->c_r.len) + c->ead_2.len);
		BYTE_ARRAY_NEW(ciphertext_2, CIPHERTEXT2_PSK_SIZE, plaintext_2.len);

		TRY(ciphertext_gen_psk(CIPHERTEXT2_PSK, &rc->suite, &c->c_r, &NULL_ARRAY, &NULL_ARRAY, &c->ead_2, &PRK_2e, &th2,
				   &ciphertext_2, &plaintext_2));
		...
		TRY(th3_psk_calculate(rc->suite.edhoc_hash, &th2, &plaintext_2,
			&rc->th3));
	}
		      ...
		      struct byte_array *initiator_pk)
{
	...
	if (!rc->is_psk){
		...
		BYTE_ARRAY_NEW(ctxt3a, CIPHERTEXT3A_PSK_SIZE, rc->msg.len);
		TRY(decode_bstr(&rc->msg, &ctxt3a));
		PRINT_ARRAY("CIPHERTEXT_3A", ctxt3a.ptr, ctxt3a.len);

		BYTE_ARRAY_NEW(ptxt3a,
			       PLAINTEXT3A_PSK_SIZE,
			       ctxt3a.len);

		BYTE_ARRAY_NEW(id_cred_psk, ID_CRED_PSK_SIZE, ID_CRED_PSK_SIZE);
		BYTE_ARRAY_NEW(ctxt3b, CIPHERTEXT3B_PSK_SIZE, CIPHERTEXT3B_PSK_SIZE);

		TRY(ciphertext_decrypt_split_psk(CIPHERTEXT3A_PSK, &rc->suite, NULL, &id_cred_psk, &ctxt3b,
			NULL, &rc->prk_3e2m,
			&rc->th3, &ctxt3a, &ptxt3a));

		//Retrieving CRED_I and CRED_R using the received ID_CRED_PSK. 
		BYTE_ARRAY_NEW(cred_i, CRED_I_SIZE, CRED_I_SIZE);
		BYTE_ARRAY_NEW(cred_r, CRED_R_SIZE, CRED_R_SIZE);
		enum err res_get_creds = get_creds_by_id(&id_cred_psk, &cred_i, &cred_r);

		if (res_get_creds == not_implemented){
			/*Reading creds from test vectors (no resumption), if the received id_cred_psk is not equals to the expected id_cred_psk, error*/
			if (!array_equals(&id_cred_psk, &c->id_cred_psk)){
				return credential_not_found;
			}
			memcpy(cred_i.ptr, cred_i_array->ptr->cred.ptr, cred_i_array->ptr->cred.len);
			cred_i.len = cred_i_array->ptr->cred.len;

			memcpy(cred_r.ptr, c->cred_r.ptr, c->cred_r.len);
			cred_r.len = c->cred_r.len;
		}	
		else if (res_get_creds != ok){
			/*an error occurred during psk retrieval*/
			return res_get_creds;
		}

		PRINT_ARRAY("cred_i obtained", cred_i.ptr, cred_i.len);
		PRINT_ARRAY("cred_r obtained", cred_r.ptr, cred_r.len);

		// EXTRACT K VALUE FROM CRED_R, ASSUMING THAT CRED_R IS A CWT.
		BYTE_ARRAY_NEW(k_cred_psk, K_CRED_PSK_SIZE, K_CRED_PSK_SIZE);

		size_t decode_len = 0;
		struct CWT cwt;

		// decode CWT cred with COSE_Key
		TRY_EXPECT(cbor_decode_CWT(cred_r.ptr, cred_r.len, &cwt, &decode_len), 0);

		TRY(_memcpy_s(k_cred_psk.ptr, k_cred_psk.len, cwt.CWT_Claim_Key_m.Claim_Key_COSE_Key_m.COSE_Key_nintbstr.value,
			      (uint32_t)cwt.CWT_Claim_Key_m.Claim_Key_COSE_Key_m.COSE_Key_nintbstr.len));
		k_cred_psk.len = (uint32_t)cwt.CWT_Claim_Key_m.Claim_Key_COSE_Key_m.COSE_Key_nintbstr.len;

		PRINT_ARRAY("k_cred_psk", k_cred_psk.ptr, k_cred_psk.len);

		/*derive prk_4e3m*/
		TRY(prk_derive_psk(rc->suite, SALT_4e3m, &rc->th3,
			       &rc->prk_3e2m, &k_cred_psk, rc->prk_4e3m.ptr));
		PRINT_ARRAY("prk_4e3m", rc->prk_4e3m.ptr, rc->prk_4e3m.len);

		#if defined(_WIN32)
		BYTE_ARRAY_NEW(ptxt3b,
			       PLAINTEXT3B_PSK_SIZE + 16, // 16 is max aead mac length
			       ctxt3b.len);
		#else
			BYTE_ARRAY_NEW(ptxt3b,
				       PLAINTEXT3B_PSK_SIZE + get_aead_mac_len(rc->suite.edhoc_aead),
				       ctxt3b.len);
		#endif
		
		TRY(ciphertext_decrypt_split_psk(CIPHERTEXT3B_PSK, &rc->suite, NULL, NULL, NULL,
			&rc->ead, &rc->prk_4e3m,
			&rc->th3, &ctxt3b, &ptxt3b));

		/*TH4*/
		// ptxt3.len = ptxt3.len - get_aead_mac_len(rc->suite.edhoc_aead);
		TRY(th4_psk_calculate(rc->suite.edhoc_hash, &rc->th3, &id_cred_psk, &ptxt3b, &cred_i, &cred_r,
				   &rc->th4));
	...
	enum err (*ead_process)(void *params, struct byte_array *ead13))
{
	...
	// Byte added following CoAP-EDHOC profile.
	if (*(rc.msg.ptr) != 0xF5) {
		return crypto_operation_not_implemented;
	}
	rc.msg.ptr++;
	rc.msg.len--;
	PRINT_ARRAY("MSG1 PAYLOAD", rc.msg.ptr, rc.msg.len);
	...
	// CHECK IF THE RECEIVED C_R MATCHES THE STORED ONE. COMPLIANCE WITH RFC 9668.
	BYTE_ARRAY_NEW(received_c_r, C_R_SIZE, C_R_SIZE);
	memcpy(received_c_r.ptr, rc.msg.ptr, C_R_SIZE);
	received_c_r.len = C_R_SIZE;

	if (!array_equals(&received_c_r, &c->c_r)){
		return wrong_parameter;
	}
	PRINTF("PREPENDED C_R MATCHING\n");

	rc.msg.ptr++;
	rc.msg.len--;
	PRINT_ARRAY("MSG3 PAYLOAD", rc.msg.ptr, rc.msg.len);
