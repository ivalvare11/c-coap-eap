...
int internal_main(void)
{
	...
	BYTE_ARRAY_NEW(CRED_PSK_out, 32, 32);
	BYTE_ARRAY_NEW(ID_CRED_PSK_out, 4, 4);
...
#define ORIG
	...
	if (TEST_VEC_NUM > 6){
	   ...
	   c_i.cred_psk.ptr = (uint8_t *)malloc(CRED_PSK_SIZE);

		if (get_last_psk(&c_i.id_cred_psk, &c_i.cred_psk, "../../../test_vectors/psks_i.txt")!=0)
			return -1;
		*/
	...
	//PSK DERIVATION.
	edhoc_exporter(SHA_256, CRED_PSK, &prk_exporter,
				   &CRED_PSK_out);
	PRINT_ARRAY("new CRED_PSK: ", CRED_PSK_out.ptr,
			    CRED_PSK_out.len);

	edhoc_exporter(SHA_256, ID_CRED_PSK, &prk_exporter,
				   &ID_CRED_PSK_out);
	PRINT_ARRAY("new ID_CRED_PSK: ", ID_CRED_PSK_out.ptr,
			    ID_CRED_PSK_out.len);

	/*
	//Storing PSK.
	store_psk(&ID_CRED_PSK_out, &CRED_PSK_out, "../../../test_vectors/psks_i.txt");
	*/
	...
	/*
	if (TEST_VEC_NUM > 6){
		free(c_i.id_cred_psk.ptr);
		free(c_i.cred_psk.ptr);
	}
	*/
