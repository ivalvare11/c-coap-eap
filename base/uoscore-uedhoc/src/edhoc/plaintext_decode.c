...
#include "cbor/edhoc_psk_decode_plaintext2.h"
#include "cbor/edhoc_psk_decode_plaintext3a.h"
#include "cbor/edhoc_psk_decode_plaintext3b.h"
...
static enum err plaintext2_psk_split(struct byte_array *ptxt,
				 struct byte_array *c_r,
				 struct byte_array *ead)
{
	size_t decode_len = 0;
	...
	TRY_EXPECT(cbor_decode_ptxt2_psk(ptxt->ptr, ptxt->len, &p, &decode_len), 0);

	/*decode C_R*/
	if (p.ptxt2_psk_C_R_choice == ptxt2_psk_C_R_bstr_c) {
		TRY(_memcpy_s(c_r->ptr, c_r->len, p.ptxt2_psk_C_R_bstr.value,
			      (uint32_t)p.ptxt2_psk_C_R_bstr.len));
		c_r->len = (uint32_t)p.ptxt2_psk_C_R_bstr.len;
	} else {
		/*provide C_R in encoded form if it was an int*/
		/*this is how it C_R was chosen by the responder*/
		TRY(encode_int(&p.ptxt2_psk_C_R_int, 1, c_r));
	}

	if (p.ptxt2_psk_EAD_2_present == true) {
		TRY(_memcpy_s(ead->ptr, ead->len, p.ptxt2_psk_EAD_2.value,
			      (uint32_t)p.ptxt2_psk_EAD_2.len));
		ead->len = (uint32_t)p.ptxt2_psk_EAD_2.len;
	} else {
		if (ead->len) {
			ead->len = 0;
		}
...
static enum err plaintext3b_psk_split(struct byte_array *ptxt,
				 struct byte_array *ead)
{
	size_t decode_len = 0;
	struct ptxt3b_psk_EAD_3 p;

	TRY_EXPECT(cbor_decode_ptxt3b_psk_EAD_3(ptxt->ptr, ptxt->len, &p, &decode_len), 0);

	if (p.ptxt3b_psk_EAD_3_present == true) {
		TRY(_memcpy_s(ead->ptr, ead->len, p.ptxt3b_psk_EAD_3.value,
			      (uint32_t)p.ptxt3b_psk_EAD_3.len));
		ead->len = (uint32_t)p.ptxt3b_psk_EAD_3.len;
	} else {
		if (ead->len) {
			ead->len = 0;
		}
...
static enum err plaintext3a_psk_split(struct byte_array *ptxt,
				 struct byte_array *id_cred_psk, struct byte_array *ciphertext_b)
{
	...
	TRY_EXPECT(cbor_decode_ptxt3a_psk(ptxt->ptr, ptxt->len, &p, &decode_len), 0);

	/*ID_CRED_PSK*/
	if (p.ptxt3a_psk_ID_CRED_PSK_choice == ptxt3a_psk_ID_CRED_PSK_bstr_c) {
		TRY(_memcpy_s(id_cred_psk->ptr, id_cred_psk->len, p.ptxt3a_psk_ID_CRED_PSK_bstr.value,
			      (uint32_t)p.ptxt3a_psk_ID_CRED_PSK_bstr.len));
		id_cred_psk->len = (uint32_t)p.ptxt3a_psk_ID_CRED_PSK_bstr.len;
	} else {
		/*provide id_cred_psk in encoded form if it was an int*/
		/*this is how it id_cred_psk was chosen by the responder*/
		TRY(encode_int(&p.ptxt3a_psk_ID_CRED_PSK_int, 1, id_cred_psk));
	}

	/*CIPHERTEXT_3B*/
	TRY(_memcpy_s(ciphertext_b->ptr, ciphertext_b->len,
		      p.ptxt3a_psk_CIPHERTEXT_3B.value,
		      (uint32_t)p.ptxt3a_psk_CIPHERTEXT_3B.len));
	ciphertext_b->len = (uint32_t)p.ptxt3a_psk_CIPHERTEXT_3B.len;
	
	return ok;
}

enum err plaintext_split_psk(struct byte_array *ptxt, struct byte_array *c_r,
			 struct byte_array *id_cred_psk,  struct byte_array *ciphertext_b, struct byte_array *ead)
{
	/*C_R is present only in plaintext 2*/
	if (c_r != NULL) {
		return plaintext2_psk_split(ptxt, c_r, ead);
	} else if (id_cred_psk != NULL) {
		return plaintext3a_psk_split(ptxt, id_cred_psk, ciphertext_b);
	} else {
		return plaintext3b_psk_split(ptxt, ead);
	}
}