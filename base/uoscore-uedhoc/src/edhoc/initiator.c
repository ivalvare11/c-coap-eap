...
static enum err msg2_process(const struct edhoc_initiator_context *c,
			     ...
			     bool static_dh_r, bool is_psk, struct byte_array *th3,
			     struct byte_array *PRK_3e2m)
{
	...
	if (!is_psk){
		ciphertext_size = CIPHERTEXT2_SIZE;
		plaintext_size = PLAINTEXT2_SIZE;
	} else {
		ciphertext_size = CIPHERTEXT2_PSK_SIZE;
		plaintext_size = PLAINTEXT2_PSK_SIZE;
	}

	BYTE_ARRAY_NEW(ciphertext, ciphertext_size, ciphertext_len);
	BYTE_ARRAY_NEW(plaintext, plaintext_size, ciphertext.len);
	...
	if (!is_psk){
		...
		TRY(check_buffer_size(PLAINTEXT2_PSK_SIZE, plaintext.len));
		
		TRY(ciphertext_decrypt_split_psk(CIPHERTEXT2_PSK, &rc->suite, c_r, NULL, NULL,
					     &rc->ead, &PRK_2e, &th2,
					     &ciphertext, &plaintext));
		...
		TRY(th3_psk_calculate(rc->suite.edhoc_hash, &th2, &plaintext,
				   th3));

		// EXTRACT K VALUE FROM CRED_I, ASSUMING THAT CRED_I IS A CWT.
		...
		TRY_EXPECT(cbor_decode_CWT(c->cred_i.ptr, c->cred_i.len, &cwt, &decode_len), 0);
		...
		/*derive prk_4e3m*/
		TRY(prk_derive_psk(rc->suite, SALT_4e3m, th3, PRK_3e2m, &k_cred_psk, rc->prk_4e3m.ptr));
		PRINT_ARRAY("prk_4e3m", rc->prk_4e3m.ptr, rc->prk_4e3m.len);
...
static enum err msg3_psk_only_gen(const struct edhoc_initiator_context *c,
			      ...
			      struct byte_array *prk_out)
{
	...
	/*Ciphertext 3b calculate*/
	BYTE_ARRAY_NEW(ciphertext_b, CIPHERTEXT3B_PSK_SIZE, CIPHERTEXT3B_PSK_SIZE);
#if PLAINTEXT3B_PSK_SIZE != 0
	BYTE_ARRAY_NEW(plaintext_b, PLAINTEXT3B_PSK_SIZE, PLAINTEXT3B_PSK_SIZE);
#else
	struct byte_array plaintext_b = BYTE_ARRAY_INIT(NULL, 0);
#endif

	BYTE_ARRAY_NEW(plaintext_a, PLAINTEXT3A_PSK_SIZE,
		       AS_BSTR_SIZE(c->id_cred_psk.len) + AS_BSTR_SIZE(ciphertext_b.len));
	BYTE_ARRAY_NEW(ciphertext_a, CIPHERTEXT3A_PSK_SIZE,
		       plaintext_a.len);

	/*create plaintext3b and ciphertext3b*/
	TRY(ciphertext_gen_psk(CIPHERTEXT3B_PSK, &rc->suite, &NULL_ARRAY, &NULL_ARRAY, &NULL_ARRAY,
				&c->ead_3, &rc->prk_4e3m, th3,
			   &ciphertext_b, &plaintext_b));

	/*create plaintext3a and ciphertext3a*/
	TRY(ciphertext_gen_psk(CIPHERTEXT3A_PSK, &rc->suite, &NULL_ARRAY, &c->id_cred_psk, &ciphertext_b,
			   &NULL_ARRAY, PRK_3e2m, th3,
			   &ciphertext_a, &plaintext_a));

	/*message 3 create and send*/
	TRY(encode_bstr(&ciphertext_a, &rc->msg));
	PRINT_ARRAY("msg3", rc->msg.ptr, rc->msg.len);

	/*TH4*/
	TRY(th4_psk_calculate(rc->suite.edhoc_hash, th3, &c->id_cred_psk, &plaintext_b, &c->cred_i, &cred_r_array->ptr->cred, 
			   &rc->th4));
...
{
	bool static_dh_i = false, static_dh_r = false, is_psk = false;
	authentication_type_get(c->method, &static_dh_i, &static_dh_r, &is_psk);
	BYTE_ARRAY_NEW(th3, HASH_SIZE, HASH_SIZE);
	BYTE_ARRAY_NEW(PRK_3e2m, PRK_SIZE, PRK_SIZE);

	/*process message 2*/
	TRY(msg2_process(c, rc, cred_r_array, c_r, static_dh_i, static_dh_r, is_psk,
			 &th3, &PRK_3e2m));

	/*generate message 3*/
	if (is_psk){
		msg3_psk_only_gen(c, rc, cred_r_array, &th3, &PRK_3e2m, prk_out);
	} else {
	...
	enum err (*ead_process)(void *params, struct byte_array *ead24))
{
	...
	// Allocate new buffer with room for 0xF5 (RFC9668)
	BYTE_ARRAY_NEW(msg_coap_head, rc.msg.len + 1, rc.msg.len + 1);
	
	// Prepend 0xF5 and copy the rest of the message
	msg_coap_head.ptr[0] = 0xF5;
	memcpy(msg_coap_head.ptr + 1, rc.msg.ptr, rc.msg.len);

	TRY(tx(c->sock, &msg_coap_head));
	...
	// Allocate new buffer with room for C_R (RFC9668)
	BYTE_ARRAY_NEW(msg_cr_head, rc.msg.len + C_R_SIZE, rc.msg.len + C_R_SIZE);

	// Prepend C_R and copy the rest of the message
	memcpy(msg_cr_head.ptr, c_r_bytes->ptr, c_r_bytes->len);
	memcpy(msg_cr_head.ptr + c_r_bytes->len, rc.msg.ptr, rc.msg.len);

	TRY(tx(c->sock, &msg_cr_head));
