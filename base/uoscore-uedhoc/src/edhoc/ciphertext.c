...
static enum err key_gen_psk(enum ciphertext_psk ctxt, enum hash_alg edhoc_hash,
			struct byte_array *prk, struct byte_array *th,
			struct byte_array *key, struct byte_array *iv)
{
	switch (ctxt) {
	case CIPHERTEXT2_PSK:
		TRY(edhoc_kdf(edhoc_hash, prk, KEYSTREAM_2, th, key));
		PRINT_ARRAY("KEYSTREAM_2", key->ptr, key->len);
		break;
	case CIPHERTEXT3A_PSK:
		TRY(edhoc_kdf(edhoc_hash, prk, KEYSTREAM_3, th, key));
		PRINT_ARRAY("KEYSTREAM_3", key->ptr, key->len);
		break;
	case CIPHERTEXT3B_PSK:
		PRINT_ARRAY("PRK_4e3m", prk->ptr, prk->len);
		PRINT_ARRAY("TH_3", th->ptr, th->len);
		TRY(edhoc_kdf(edhoc_hash, prk, K_3, th, key));
		PRINT_ARRAY("K_3", key->ptr, key->len);
		TRY(edhoc_kdf(edhoc_hash, prk, IV_3, th, iv));
		PRINT_ARRAY("IV_3", iv->ptr, iv->len);
...
static enum err ciphertext_encrypt_decrypt_psk(
	enum ciphertext_psk ctxt, enum aes_operation op,
	const struct byte_array *in, const struct byte_array *key,
	struct byte_array *nonce, const struct byte_array *aad,
	struct byte_array *out, struct byte_array *tag)
{
	if (ctxt != CIPHERTEXT3B_PSK) {
		xor_arrays(in, key, out);
	} else {
		PRINT_ARRAY("in", in->ptr, in->len);
		TRY(aead(op, in, key, nonce, aad, out, tag));
...
enum err ciphertext_decrypt_split_psk(
	...
	struct byte_array *ciphertext, struct byte_array *plaintext)
{
	/*generate key (and iv for ciphertext 3b)*/
	uint32_t key_len;
	if (ctxt != CIPHERTEXT3B_PSK) {
		key_len = plaintext->len;
	} else {
	...
	if (ctxt == CIPHERTEXT2_PSK) {
		key_size = CIPHERTEXT2_PSK_SIZE;
	} else if (ctxt == CIPHERTEXT3A_PSK) {
		key_size = CIPHERTEXT3A_PSK_SIZE;
	} else {
		key_size = CIPHERTEXT3B_PSK_SIZE;
	}
	BYTE_ARRAY_NEW(key, key_size, key_len);
	BYTE_ARRAY_NEW(iv, AEAD_IV_SIZE, get_aead_iv_len(suite->edhoc_aead));
	...
	struct byte_array tag = BYTE_ARRAY_INIT(ciphertext->ptr, tag_len);
	TRY(ciphertext_encrypt_decrypt_psk(ctxt, DECRYPT, ciphertext, &key, &iv,
				       &associated_data, plaintext, &tag));

	PRINT_ARRAY("plaintext", plaintext->ptr, plaintext->len);

	//Splitting plaintexts.
	if (ctxt == CIPHERTEXT2_PSK) {
		TRY(plaintext_split_psk(plaintext, c_r, NULL, NULL,
					    ead));
		PRINT_ARRAY("C_R (raw)", c_r->ptr, c_r->len);
		if (ead->len) {
			PRINT_ARRAY("ead", ead->ptr, ead->len);
		...
		TRY(plaintext_split_psk(plaintext, NULL, id_cred_psk, ciphertext_b, NULL));
		PRINT_ARRAY("ID_CRED_PSK", id_cred_psk->ptr, id_cred_psk->len);
		PRINT_ARRAY("CIPHERTEXT_3B", ciphertext_b->ptr, ciphertext_b->len);
	} else {
		TRY(plaintext_split_psk(plaintext, NULL, NULL, NULL,
			ead));
		PRINT_ARRAY("ead", ead->ptr, ead->len);
	}
			...
			struct byte_array *plaintext)
{
	...
	if (ctxt == CIPHERTEXT3A_PSK) {

		if (c_x_is_encoded_int(id_cred_psk)) {
			TRY(byte_array_append(plaintext, id_cred_psk,
					      ptxt_buf_capacity));
		} else {
			BYTE_ARRAY_NEW(id_cred_psk_enc, AS_BSTR_SIZE(ID_CRED_PSK_SIZE),
				       AS_BSTR_SIZE(id_cred_psk->len));
			TRY(encode_bstr(id_cred_psk, &id_cred_psk_enc));
			TRY(byte_array_append(plaintext, &id_cred_psk_enc,
					      ptxt_buf_capacity));
		}

		BYTE_ARRAY_NEW(ciphertext_b_enc, AS_BSTR_SIZE(CIPHERTEXT3B_PSK_SIZE),
				       AS_BSTR_SIZE(ciphertext_b->len));
		TRY(encode_bstr(ciphertext_b, &ciphertext_b_enc));
		TRY(byte_array_append(plaintext, &ciphertext_b_enc,
					      ptxt_buf_capacity));
	...
	TRY(ciphertext_encrypt_decrypt_psk(ctxt, ENCRYPT, plaintext, &key, &iv,
				       &aad, ciphertext, &tag));
	ciphertext->len += tag.len;

	PRINT_ARRAY("ciphertext_psk_2/3a/3b", ciphertext->ptr, ciphertext->len);
	return ok;
}